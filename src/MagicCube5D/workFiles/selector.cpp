...
void CSelector::unProjectMousePoint(int mouseX, int mouseY, CVector3D& linep1,
                                    CVector3D& linep2) {
    ...
    // Do the unprojects.
    gluUnProject((GLdouble)x, (GLdouble)y, 0.0, mvmatrix, projmatrix, viewport, &wx,
                 &wy, &wz);
    linep1 = CVector3D(wx, wy, wz);

    // TRACE( "World coordinates at z=0.0 are (%f,%f,%f)\n", wx, wy, wz );
    gluUnProject((GLdouble)x, (GLdouble)y, 1.0, mvmatrix, projmatrix, viewport, &wx,
                 &wy, &wz);
