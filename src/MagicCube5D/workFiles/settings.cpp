...
void CSettings::cycleAccentSetting(EStickerAccent& accent) const {
    switch (accent) {
        case DIM:
            accent = NORMAL;
            break;
...
void CSettings::setAllVisible() { m_invisibleFaces.clear(); }

void CSettings::setFaceVisible(int f, bool visible) {
    if (false == visible) {
        for (uint i = 0; i < m_invisibleFaces.size(); i++)
            if (f == m_invisibleFaces[i]) return;
...
bool CSettings::getFaceVisible(int f) const {
    for (uint j = 0; j < m_invisibleFaces.size(); j++)
        if (f == m_invisibleFaces[j]) return false;
