...
CubeFrame::MouseMove(System::Object^  sender, System::Windows::Forms::MouseEventArgs^  e) 
{
	...
	// NOTE: The mousedown checks makes sure we had a mouse down call and fixes a problem I was seeing
...
CubeFrame::MouseUp(System::Object^  sender, System::Windows::Forms::MouseEventArgs^  e) 
{
	...
	if( e->Button == System::Windows::Forms::MouseButtons::Left )
	{
		...
		if( CCube5D::READY_FOR_FIRST == m_renderer->m_cube.m_clickState )
			...
			rotateStarted = m_renderer->m_cube.handleSecondClick( CtrlDown(), 0 == m_slicemask ? 1 : m_slicemask );
		if( rotateStarted )
		{
			m_currentAngle = 0;
			m_animating = true;
...
CubeFrame::KeyDown(System::Object^  sender, System::Windows::Forms::KeyEventArgs^  e) 
{
	...
	if( e->KeyCode == Keys::Escape )
		m_renderer->m_cube.resetClickState();

	// Cycle through click stickers.
	if( e->KeyCode == Keys::Left )
		m_renderer->m_cube.cycleSelectedGotoSticker( true );
	if( e->KeyCode == Keys::Right )
		m_renderer->m_cube.cycleSelectedGotoSticker( false );
...
void 
CubeFrame::KeyUp(System::Object^  sender, System::Windows::Forms::KeyEventArgs^  e) 
{
	if( m_animating )
		return;
...
void 
CubeFrame::SliceDown(System::Object^  sender, System::Windows::Forms::KeyEventArgs^  e) 
{
...
void 
CubeFrame::SliceUp(System::Object^  sender, System::Windows::Forms::KeyEventArgs^  e) 
{
	// Update our slicemask;
	if( e->KeyCode == Keys::NumPad1 || e->KeyCode == Keys::D1 )
		m_slicemask &= ~SLICEMASK_1;
...
CubeFrame::CycleStickerAccent( int colors ) 
{
	...
	{
	case 1:
		m_renderer->m_cube.m_settings.cycleAccentSetting( m_renderer->m_cube.m_settings.m_accent1Colored );
		break;
	case 2:
		m_renderer->m_cube.m_settings.cycleAccentSetting( m_renderer->m_cube.m_settings.m_accent2Colored );
		break;
	case 3:
		m_renderer->m_cube.m_settings.cycleAccentSetting( m_renderer->m_cube.m_settings.m_accent3Colored );
		break;
	case 4:
		m_renderer->m_cube.m_settings.cycleAccentSetting( m_renderer->m_cube.m_settings.m_accent4Colored );
		break;
	case 5:
		m_renderer->m_cube.m_settings.cycleAccentSetting( m_renderer->m_cube.m_settings.m_accent5Colored );
		break;
	}
...
void 
CubeFrame::Rotate( int a1, int a2, int face, int dummy ) 
{
	if( m_animating )
		return;
...
CubeFrame::Redo() 
{
	...
	STwist redo;
	if( m_renderer->m_cube.getRedoParams( redo ) )
		internalRotate( redo );
}
...
void 
CubeFrame::HighlightCubies( List<int> ^ colors ) 
{
...
//		 e.g. changing a name could update the UI but not the macro file as it is now.
void 
CubeFrame::StartMacroRecord() 
{
	if( m_animating )
		return;
...
void 
CubeFrame::RenameMacro( int index, String ^ name ) 
{
	if( m_animating )
		return;
...
void 
CubeFrame::DeleteMacro( int index ) 
{
	if( m_animating )
		return;
...
void 
CubeFrame::ExecuteMacro( int index, bool reverse ) 
{
	if( m_animating )
		return;
	...
	// Show a congratulatory message if they've solved it.
	if( m_renderer->m_cube.showSolvedMessage() )
		showSolvedMessage();
}
...
void 
CubeFrame::internalSelectSticker() 
{
