...
private:
	...
	void draw4Cube( const CVector5D & offset, int axis1, int axis2, int axis3, int axis4, double scale = 1.0, bool rotate = true ) const;
	
	// Get the vertices of a 4-cube.
	void get4CubeVertices( const CVector5D & offset, int axis1, int axis2, int axis3, int axis4, double scale, CVector5D vertices[16] ) const;

	// Get a sticker's 5D projected center.
	// This is not the normalized center (it takes into account all the projection settings).
	...
	void getSticker3Extents( const SSticker & sticker, CVector3D & low, CVector3D & high ) const;
	...
	// Twisting helpers.
	// getRotationInfo finds the axis and the values on that axis a sticker can have to be rotated.
	void getRotationInfo( int & axis, std::vector<double> & values ) const;
	bool shouldWeRotate( const SSticker & sticker, int axis, const std::vector<double> & values ) const;
	bool shouldWeRotate( const CVector5D & cubie, int axis, const std::vector<double> & values ) const;

	// Internal sticker selection helper.
	void selectStickerHelper( const CVector3D & linep1, const CVector3D & linep2, bool shift,
