...
CLoader::loadFromFile( StateMatrix & state, std::vector<STwist> & twists ) 
{
	...
	if( ! tempString->Equals( "MagicCube5D" ) )
		return false;

	myEnum->MoveNext();
	tempString = safe_cast<String ^>( myEnum->Current );
	if( ! tempString->Equals( VERSION_STRING ) )
		return false;
...
void 
CLoader::saveMacrosToFile( const std::vector<SMacro> & macros ) 
{
...
bool 
CLoader::loadMacrosFromFile( std::vector<SMacro> & macros ) 
{
	macros.clear();
...
void 
CLoader::saveTwists( System::IO::StreamWriter ^ sw, const std::vector<STwist> & twists ) 
{
	String ^ line = "";
	int count = 0;
	...
	{
		String ^ twistString = 
			-1 == twists[t].m_rotationFace ? "v" : System::Convert::ToString( twists[t].m_rotationFace );
		twistString += 
			System::Convert::ToString( twists[t].m_rotationAxis1 ) + 
...
bool 
CLoader::loadTwists( System::IO::StreamReader ^ sr, std::vector<STwist> & twists, int numLines ) 
{
	twists.clear();
    ...
    while( sr->Peek() >= 0 ) 
    {
		...
		{
			String ^ tempString = safe_cast<String ^>( myEnum->Current );
			if( tempString->Length < 3 || tempString->Length > 6 )
				return false;

			STwist twist;
			twist.m_rotationFace = "v" == tempString->Substring( 0, 1 ) ? 
				-1 : System::Convert::ToInt32( tempString->Substring( 0, 1 ) );
			twist.m_rotationAxis1 = System::Convert::ToInt32( tempString->Substring( 1, 1 ) );
			twist.m_rotationAxis2 = System::Convert::ToInt32( tempString->Substring( 2, 1 ) );

			// Handle loading a slicemask.
			if( tempString->Length > 3 )
				twist.m_slicemask = System::Convert::ToInt32( tempString->Substring( 4, tempString->Length - 4 ) );
