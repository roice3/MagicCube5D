...
CCube5D::CCube5D() : m_transformer(m_settings) {
    ...
    for (int i = 0; i < 10; ++i) m_colors.push_back(CColor(1, 1, 1, 1));
}

void CCube5D::render(const CVector3D& lookFrom) {
    ...
    switch (m_settings.m_stereoMode) {
        ...
        case STEREO_CROSS_EYED: {
            ...
            // The projection naturally "rotates" the two images some because of the
            // offset. This is a correction angle for that effect.
            ...
            glRotated(-m_settings.m_stereoSeparation + correction, axis.m_components[0],
                      axis.m_components[1], axis.m_components[2]);
            renderCube();
            glPopMatrix();
        ...
        case STEREO_ANAGLYPH_NO_COLOR: {
            ...
            // NOTE: We could take this out or make it a completely separate option with
            // a separate slider. It effectively moves everything in or out of the
            // screen (negative values move everything into the screen). I played with
            // different values, and this seemed pretty good since it pops it out of the
            // screen somewhat.
            // offset.empty();
            ...
            glPushMatrix();
            glTranslated(-offset.m_components[0], -offset.m_components[1], 0);
            glRotated(-m_settings.m_stereoSeparation, axis.m_components[0],
                      axis.m_components[1], axis.m_components[2]);
            renderCube();
            glPopMatrix();
...
void CCube5D::setPuzzleType(int n) {
    m_settings.m_n = n;
    reset();
}

void CCube5D::setFaceVisible(int i, bool visible) {
    m_settings.setFaceVisible(i, visible);
    calculateVisibilities();
}

bool CCube5D::getFaceVisible(int i) const { return (m_settings.getFaceVisible(i)); }

void CCube5D::setFaceColor(int i, CColor c) {
    m_colors[i] = c;
...
CColor CCube5D::getFaceColor(int i) { return m_colors[i]; }

void CCube5D::startRotate(const STwist& twist) {
    m_rotation = m_percentageDone = 0;
    m_currentTwist = twist;
    calculateVisibilities();
}

void CCube5D::iterateRotate() {
    m_rotation += deg2rad(m_settings.m_rotationStep);
    m_percentageDone = m_rotation / deg2rad(90.0);
}

void CCube5D::finishRotate() {
    ...
    if (m_undoMode) {
        ...
        if (m_currentlyRecording) m_recordingMacro.m_twists.pop_back();
    ...
    if (m_currentlyRecording) m_recordingMacro.m_twists.push_back(m_currentTwist);
}

bool CCube5D::getUndoParams(STwist& twist) {
    if (!m_twists.size()) return false;
...
bool CCube5D::getRedoParams(STwist& twist) {
    if (!m_redoTwists.size()) return false;
...
void CCube5D::getMacroNames(std::vector<std::string>& names) {
    names.clear();

    // Load the macros from file.
    m_loader.loadMacrosFromFile(m_macros);
    for (uint i = 0; i < m_macros.size(); i++) names.push_back(m_macros[i].m_name);
}

void CCube5D::startMacroRecord() {
    assert(!m_currentlyRecording);
    m_recordingMacro = SMacro();
    m_currentlyRecording = true;
}

void CCube5D::stopMacroRecord() {
    assert(m_currentlyRecording);
    m_currentlyRecording = false;
...
void CCube5D::renameMacro(int index, std::string name) {
...
void CCube5D::deleteMacro(int index) {
...
void CCube5D::executeMacro(int index, bool reverse) {
...
bool CCube5D::handleFirstClick(bool control) {
    if (m_clickState != READY_FOR_FIRST) {
        assert(false);
        return false;
    }

    if (m_selectedSticker == SSticker()) return false;

    if (control) return (handleControlClick());
...
bool CCube5D::handleSecondClick(bool control, int slicemask) {
    ...
    twist.m_rotationFace =
            m_transformer.getFaceFromOffset(m_firstClickedSticker.m_offset);
    ...
    if (temp1.m_components[twist.m_rotationAxis1] *
                temp2.m_components[twist.m_rotationAxis2] <
        0)
        twist.reverseTwist();
...
bool CCube5D::handleControlClick() {
    if (m_selectedSticker == SSticker()) return false;
    ...
    // Do we need to reverse the rotation (is the face offset in the positive
    // direction)?
...
void CCube5D::cycleSelectedGotoSticker(bool left) {
    if (m_clickState != READY_FOR_SECOND) return;
...
void CCube5D::getGotoStickers() {
    m_gotoStickers.clear();

    if (m_firstClickedSticker == SSticker()) return;

    for (uint i = 0; i < m_stickers.size(); i++) {

        // Has to be on the same face.
        if (m_stickers[i].m_offset != m_firstClickedSticker.m_offset) continue;

        // Has to be a 2-colored piece.
        if (2 != m_stickers[i].getNumColors()) continue;

        // We're not interested in ourself or stickers on the other side.
        CVector5D temp1 =
                m_firstClickedSticker.m_cubie - m_firstClickedSticker.m_offset;
        CVector5D temp2 = m_stickers[i].m_cubie - m_stickers[i].m_offset;
        if (temp1 == temp2 || temp1 == temp2 * -1) continue;
        ...
        for (int axis = 0; axis < 5; axis++) {
            if (axis == axis1 || axis == faceAxis) continue;
...
void CCube5D::load() {
    StateMatrix state;
    if (!m_loader.loadFromFile(state, m_twists, &m_scrambled)) return;
...
void CCube5D::selectStickerHelper(const CVector3D& linep1, const CVector3D& linep2,
    ...
    if (STEREO_NONE != m_settings.m_stereoMode) return;
    ...
    if (selectedSticker == SSticker()) return;
    ...
    if (mustIntersect) {
        ...
        double distance = (((CVector3D)(position - linep1)).cross(lineNormal)).abs() /
                          lineNormal.abs();
        if (distTest < distance) selectedSticker = SSticker();
    }
}

void CCube5D::scramble(int numTwists) {
    ...
    for (int i = 0; i < numTwists; i++) {
        ...
        while (!valid) {
            ...
            if (m_currentTwist.m_rotationAxis1 != axis) valid = true;
        }
...
void CCube5D::highlightCubies(std::vector<int>& colors) {
    m_transformer.findCubies(m_stickers, colors, m_cubiesToHighlight);
}

bool CCube5D::isSolved() const {
    StateMatrix state;
    m_transformer.stickersToMatrix(m_stickers, state);
    return m_transformer.isSolved(state);
}

bool CCube5D::showSolvedMessage() {

    // We only want to show the message if an actual scramble occurred.
    if (!m_scrambled) return false;

    // We only want to show a message once.
    if (m_solvedMessageShown) return false;

    // We'll heuristically check that this is a genuine solution by making sure 1500
    // twists have been done. Since there are 243 pieces total, 32 of which are corners
    // (each requiring a sequence on the order of 32 moves to place), it should be safe
    // to assume just about every manual solution will take more moves than this.
    if (m_twists.size() <= 1500) return false;
...
SSolvedInfo CCube5D::updateSolvedInfo() {
    m_transformer.getSolvedStatus(m_stickers, m_solvedInfo);
    return m_solvedInfo;
}

void CCube5D::createStickers() {
...
void CCube5D::renderCube() {
    ...
    for (uint i = 0; i < m_stickers.size(); i++) {
        ...
        if (cubieToHighlight) forceVisible = true;
        ...
        if (m_rotation == 0) {
            ...
            if (m_settings.m_showAllCubieStickers &&
                m_stickers[i].m_cubie == m_selectedSticker.m_cubie)
                forceVisible = true;

            if (!forceVisible)
                if (!m_stickers[i].m_currentVis) continue;
        } else {
            if (!forceVisible) {

                // Invisible now and in the future?
                if (!m_stickers[i].m_currentVis && !m_stickers[i].m_futureVis) continue;
            }
        }
...
void CCube5D::vertex(CVector5D& p) const {
    p.projectTo4D(m_settings.m_projection5Distance);
    p.projectTo3D(m_settings.m_projection4Distance);
    glVertex3d((GLdouble)p.m_components[0], (GLdouble)p.m_components[1],
               (GLdouble)p.m_components[2]);
}

void CCube5D::drawSticker(const SSticker& sticker, bool rotate,
                          bool cubieToHighlight) const {
    ...
    if (cubieToHighlight) alpha = 1.0;

    if (m_useStereoColor) {
        glColor4d(m_stereoColor.m_r, m_stereoColor.m_g, m_stereoColor.m_b,
                  alpha * m_stereoColor.m_a);
    } else {
        CColor color = m_colors[sticker.m_color];
        glColor4d((GLdouble)color.m_r, (GLdouble)color.m_g, (GLdouble)color.m_b,
                  (GLdouble)alpha);
    }
    ...
    if (m_clickState == READY_FOR_SECOND) {
        ...
        highlightHigh =
                sticker == m_firstClickedSticker || sticker == m_selectedGotoSticker;
...
void CCube5D::getSticker5Position(const SSticker& sticker, CVector5D& center) const {
...
void CCube5D::getSticker3Position(const SSticker& sticker, CVector3D& position) const {
    ...
    for (int i = 0; i < 3; i++) position.m_components[i] = center.m_components[i];
}

void CCube5D::getSticker3Extents(const SSticker& sticker, CVector3D& low,
    ...
    get4CubeVertices(center, axis1, axis2, axis3, axis4, m_settings.m_stickerSize,
                     vertices);
...
namespace {
void renderPoints(const CVector5D& p1, const CVector5D& p2, const CVector5D& p3,
                  const CVector5D& p4) {
    glBegin(GL_LINE_LOOP);

    glVertex3d((GLdouble)p1.m_components[0], (GLdouble)p1.m_components[1],
               (GLdouble)p1.m_components[2]);
    glVertex3d((GLdouble)p2.m_components[0], (GLdouble)p2.m_components[1],
               (GLdouble)p2.m_components[2]);
    glVertex3d((GLdouble)p3.m_components[0], (GLdouble)p3.m_components[1],
               (GLdouble)p3.m_components[2]);
    glVertex3d((GLdouble)p4.m_components[0], (GLdouble)p4.m_components[1],
...
void CCube5D::draw4Cube(const CVector5D& offset, int axis1, int axis2, int axis3,
                        int axis4, double scale, bool rotate) const {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        for (int j = 0; j < 4; j++) {
            ...
            if (rotate)
                point.rotate(m_currentTwist.m_rotationAxis1,
                             m_currentTwist.m_rotationAxis2, rotation);
...
void CCube5D::get4CubeVertices(const CVector5D& offset, int axis1, int axis2, int axis3,
...
void CCube5D::calculateVisibilities() {
    ...
    for (uint i = 0; i < m_stickers.size(); i++) {
        ...
        bool rotate = shouldWeRotate(m_stickers[i], axis, values);
        if (rotate) {
            face.rotate(m_currentTwist.m_rotationAxis1, m_currentTwist.m_rotationAxis2,
                        deg2rad(90.0));
...
void CCube5D::calculateNewStickerPositions() {
    ...
    for (uint i = 0; i < m_stickers.size(); i++) {
        ...
        m_stickers[i].m_cubie.rotate(m_currentTwist.m_rotationAxis1,
                                     m_currentTwist.m_rotationAxis2, deg2rad(90.0));
        m_stickers[i].m_offset.rotate(m_currentTwist.m_rotationAxis1,
                                      m_currentTwist.m_rotationAxis2, deg2rad(90.0));
    ...
    for (uint i = 0; i < m_cubiesToHighlight.size(); i++) {
        ...
        m_cubiesToHighlight[i].rotate(m_currentTwist.m_rotationAxis1,
                                      m_currentTwist.m_rotationAxis2, deg2rad(90.0));
...
void CCube5D::roundComponents(CVector5D& vector) const {

    // Our max component values are 1 or -1 no matter what the puzzle size,
    // so component values are quantized to certain decimals depending on the puzzle
    // size.
    double increment = m_settings.getIncrementBetweenStickers();
...
bool CCube5D::shouldWeRotate(const SSticker& sticker, int axis,
                             const std::vector<double>& values) const {
    return shouldWeRotate(sticker.m_cubie, axis, values);
}

bool CCube5D::shouldWeRotate(const CVector5D& cubie, int axis,
                             const std::vector<double>& values) const {
    ...
    for (uint i = 0; i < values.size(); i++) {
        ...
        if (IS_ZERO(values[i] - cubie.m_components[axis])) return true;
    }

    return false;
}

EStickerAccent CCube5D::getStickerAccent(const SSticker& sticker) const {
