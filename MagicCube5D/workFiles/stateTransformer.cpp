...
void CStateTransformer::stickersToMatrix(const std::vector<SSticker>& stickers,
    ...
    state.resize(10);
    for (int f = 0; f < 10; f++) state[f].resize(numPerFace);
...
void CStateTransformer::matrixToStickers(const StateMatrix& state,
...
int getFaceFromOffsetInternal(const CVector5D& offset) {
    if (offset == CVector5D(1, 0, 0, 0, 0)) return 0;
    if (offset == CVector5D(-1, 0, 0, 0, 0)) return 1;
    if (offset == CVector5D(0, 1, 0, 0, 0)) return 2;
    if (offset == CVector5D(0, -1, 0, 0, 0)) return 3;
    if (offset == CVector5D(0, 0, 1, 0, 0)) return 4;
    if (offset == CVector5D(0, 0, -1, 0, 0)) return 5;
    if (offset == CVector5D(0, 0, 0, 1, 0)) return 6;
    if (offset == CVector5D(0, 0, 0, -1, 0)) return 7;
    if (offset == CVector5D(0, 0, 0, 0, 1)) return 8;
    if (offset == CVector5D(0, 0, 0, 0, -1)) return 9;
    assert(false);
    return -1;
}
}  // namespace

void CStateTransformer::getFaceOffset(int face, CVector5D& offset) const {
    getFaceOffsetInternal(face, offset);
}

int CStateTransformer::getFaceFromOffset(const CVector5D& offset) const {
    return getFaceFromOffsetInternal(offset);
}

void CStateTransformer::getCubie(int stickerInt, const CVector5D& face,
                                 CVector5D& cubie) const {
    cubie.empty();
...
int CStateTransformer::getStickerInt(const CVector5D& cubie,
                                     const CVector5D& face) const {
    double increment = m_settings.getIncrementBetweenStickers();
    ...
    int num = m_settings.m_n;
    int stickerInt =
            (int)(roundDoubleToInt((cubie.m_components[axis1] + 1) / increment) *
                          pow((double)num, 0) +
                  roundDoubleToInt((cubie.m_components[axis2] + 1) / increment) *
                          pow((double)num, 1) +
                  roundDoubleToInt((cubie.m_components[axis3] + 1) / increment) *
                          pow((double)num, 2) +
                  roundDoubleToInt((cubie.m_components[axis4] + 1) / increment) *
                          pow((double)num, 3));
    return (stickerInt);
}

bool CStateTransformer::isSolved(const StateMatrix& state) const {
    int numPerFace = getNumPerFace();
    assert(state.size());
    assert(state[0].size());
    ...
    for (uint f = 0; f < 10; f++) {
        ...
        for (int s = 1; s < numPerFace; s++) {
            if (state[f][s] != start) return false;
        }
    }
...
// Binary Predicate for ordering CVector5Ds (so they can be used as keys to a map
// below).
struct compare5D : public std::binary_function<CVector5D, CVector5D, bool> {
    bool operator()(const CVector5D& _Left, const CVector5D& _Right) const {
        for (int i = 0; i < 5; i++) {
            if (_Left.m_components[i] < _Right.m_components[i]) return true;

            if (_Left.m_components[i] > _Right.m_components[i]) return false;
        }
...
void searchTrackedColors(__int16 colorTracker[5][5], std::vector<__int8>& foundColors,
                         std::vector<__int8>& foundFaces) {
    int max = 0, found_r = -1, found_c = -1;
    for (int r = 0; r < 5; r++) {
        std::vector<__int8>::const_iterator rIt =
                find(foundColors.begin(), foundColors.end(), r);
        if (rIt != foundColors.end()) continue;

        for (int c = 0; c < 5; c++) {
            std::vector<__int8>::const_iterator cIt =
                    find(foundFaces.begin(), foundFaces.end(), c);
            if (cIt != foundFaces.end()) continue;
...
void findCurrentFaceColorsEvenHelper(const std::vector<SSticker>& stickers,
                                     const CSettings& settings, CubieMap& colorMap,
                                     int stickerTypeToUse) {
    /*
    This is kind of a bitch!  I want the computer to do what a human could do pretty
    naturally...

    There are constraints on the ordering of the faces that should be used in this
    heuristic, e.g. certain colors have to be opposite each other, so selecting the
    color for one face effectively selects the color for two faces. Also, once 8 are
    picked, the last two are determined, so all this means really we are picking the
    best 4 face<->color matchups.

    So after some thought, what I'm going to do is generate a 5x5 matrix of numbers
    tracking the number of colors on the various faces. Columns will be "merged" faces
    and rows will be "merged" color (opposite faces will have entries put into the 5
    opposite faces based on opposite color). This means entries will range from 0 to 32,
    twice the amount of 1C pieces on a face. Once the matrix is filled, my selection
    algorithm will go like this: Find the highest number in the matrix and set the 2
    faces according to the color. Now search for the next highest number disregarding
    items from the row and column the first number was in. Now search for the next
    highest number disregarding both rows and columns for the first 2 numbers. Repeat
    this process to find the final needed 4th number.  The 5th number will then be
    determined by constraints.

    This description leaves out some of the reasoning I've made, but I think this is
    good. Specifically, it avoids problems of doing matchups that effectively only
    search for highest numbers along a row or a column, not both (which is sort of the
    natural first approach to take, e.g. search a face for which color is the most
    common, then do the matchup, but then what happens when this process also matches
    the same color to another face?).
    */

    __int16 colorTracker[5][5];
    for (int r = 0; r < 5; r++)
        for (int c = 0; c < 5; c++) colorTracker[r][c] = 0;

    // Turns out we need this too.
    // The reason to still keep around the 5x5 matrix above is to weight sets of
    // opposite faces in the choice.
    __int16 colorTrackerFull[10][10];
    for (int r = 0; r < 10; r++)
        for (int c = 0; c < 10; c++) colorTrackerFull[r][c] = 0;

    for (uint i = 0; i < stickers.size(); i++) {
        const SSticker& s = stickers[i];
        if (stickerTypeToUse != s.getNumColors()) continue;
    ...
    for (int i = 0; i < 5; i++) {
        ...
        if (i == 5) {
            ...
            needToSwitch =
                    colorTrackerFull[color1][face1] < colorTrackerFull[color2][face1];
...
void findCurrentFaceColors(const std::vector<SSticker>& stickers,
                           const CSettings& settings, CubieMap& colorMap) {

    // NOTE: This is heuristic for even puzzles, which have no static central pieces.
    // XXX - Maybe this function should also go ahead and return the number of 1C pieces
    // solved?
    ...
    switch (settings.m_n) {
        ...
        case 7: {
            for (uint i = 0; i < stickers.size(); i++) {
                ...
                if (s.m_cubie == s.m_offset) colorMap[s.m_cubie] = s.m_color;
            }
            return;
        }
        default:
...
int getNumOfCubieType(int numColors, int m_n) {
    ...
    if (m_n > 3) ret *= (int)pow((float)m_n - 2, 5 - numColors);

    return ret;
}
}  // namespace

void CStateTransformer::getSolvedStatus(const std::vector<SSticker>& stickers,
                                        SSolvedInfo& si) const {
    si.reset();
    ...
    // (because 1 bad sticker can mark this vs. tracking solved where every sticker
...
void CStateTransformer::findCubies(const std::vector<SSticker>& state,
    ...
    for (uint i = 0; i < state.size(); i++) {
        ...
        if (numColors != s.getNumColors()) continue;
        ...
        if (it != candidates.end()) numColorsFound = it->second + 1;
...
int CStateTransformer::getNumPerFace() const {
