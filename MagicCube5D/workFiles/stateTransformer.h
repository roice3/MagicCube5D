...
//
// It also has useful helpers for converting specific items between integer/verbose representations.
//
...
public:
	...
	void stickersToMatrix( const std::vector<SSticker> & stickers, StateMatrix & state ) const;
	void matrixToStickers( const StateMatrix & state, std::vector<SSticker> & stickers ) const;
	...
	void getSolvedStatus( const std::vector<SSticker> & stickers, SSolvedInfo & solvedInfo ) const;
